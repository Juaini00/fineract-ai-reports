    ...
    pub async fn route(&self, message: &str, context: &ContextWindow) -> Result<AssistantIntent> {
        ...
        let user = json!({
            ...
            "rules": [
                ...
                "entities[].entity_type MUST be exactly one of: person_name, client_id, office, date_period, currency, product, metric, capability_hint, account_number. Never invent a new entity kind. If a detail does not fit any of these, express it in constraints or omit it rather than inventing a type.",
