...
impl LlmPlannerClient {
    ...
    pub async fn choose_capability(
        ...
        let response = self
            ...
            .send()
            .await
            .with_context(|| format!("request {} planner fallback", self.provider))?;

        let status = response.status();
        if !status.is_success() {
            let body = response.text().await.unwrap_or_default();
            bail!(
                "{} planner fallback failed with status {status}: {body}",
                self.provider
            );
        }
        ...
        let payload: ChatCompletionResponse = response
            .json()
            .await
            .with_context(|| format!("parse {} planner fallback response", self.provider))?;
        let content = payload
            .choices
            .into_iter()
            .next()
            .map(|choice| choice.message.content)
            .with_context(|| format!("{} planner fallback returned no choice", self.provider))?;
