...
use chat::assistant::SwiftideIndexPipeline;

#[test]
fn indexes_project_owned_sources_only() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
    let docs = SwiftideIndexPipeline::ingest_catalog(&root).unwrap();
...
fn rejects_fineract_row_exports() {
    let root = std::env::temp_dir().join(format!("swiftide-reject-{}", uuid::Uuid::new_v4()));
    let dir = root.join("fineract_rows");
    fs::create_dir_all(&dir).unwrap();
    fs::write(dir.join("clients.yaml"), "client_id, account_no\n1, A1").unwrap();
    let error = SwiftideIndexPipeline::ingest_paths(&root).unwrap_err();
