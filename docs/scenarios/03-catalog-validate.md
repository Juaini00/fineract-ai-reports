...
- Response had `success=true`, `data.valid=true`, `data_areas=13`, `domains=7`, `capabilities=11`, and `queries=11`.
...
Expected counts after the knowledge expansion + Phase 19 savings and foundation slices:
...
- `capabilities: 11` — the 9 savings capabilities plus `organization_office_summary` and `client_lifecycle_summary`.
- `queries: 11` — one approved query per capability above.
