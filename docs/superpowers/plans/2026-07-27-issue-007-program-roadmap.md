...
| 12 (W-C) | `specs/2026-07-24-llm-extraction-gateway-design.md` (existing) | `…-b12-extraction-gateway-continuation.md` (+ existing 24-Jul plan) | ✅ | Layers 1/2/3 + `pipeline::run` (5 scenarios) + `runtime::run_via_gateway_pipeline` mapping `DecisionOutcome` to `GraphRuntimeResult` + `assistant_intent_from_extraction` bridge. `run_with_router` opts in via `AI_REPORT_GATEWAY_PIPELINE=on` env var when llm+catalog+client+canonical present. Task 7.2 done: `extract_message_facts`/`_at` demoted to `pub(crate)` verification helpers |
