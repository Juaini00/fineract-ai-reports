...
| 12 (W-C) | `specs/2026-07-24-llm-extraction-gateway-design.md` (existing) | `…-b12-extraction-gateway-continuation.md` (+ existing 24-Jul plan) | ✅ layers + pipeline + runtime entry | Layers 1/2/3 + `pipeline::run` (5 scenarios) + `runtime::run_via_gateway_pipeline` mapping `DecisionOutcome` to `GraphRuntimeResult` with 2 runtime tests. `run_with_router` default path still on classifier; Task 7.2 (demote deterministic extractor) deferred to fresh session |
