...
- **Chat auth = Bearer session JWT + `role == "admin"`, not the API key.** Every chat route uses the `AuthenticatedChatClient` extractor (`AuthenticatedUser` first); an API key can never authenticate a chat request, and job/session ownership is scoped by `user_id`, not `api_key_id`.
- **Chat runs an admin projection by design** (`project_admin_principal`): it grants all `approved_mvp` capabilities and forces `can_view_pii = true`, then intersects office scope. So in chat, `allowed_capabilities` and `can_view_pii` on an API key are **not binding**; only `allowed_office_ids` has an effect, and only as a voluntary opt-down by a cooperative caller. It is not an authorization boundary — the admin bearer already reaches full tenant, so a missing/invalid `X-API-Key` yields full tenant and must never cause a 401 (see `x_api_key_variants_do_not_change_bearer_chat_authorization`). A hard per-caller boundary would have to attach to the bearer/user identity.
- API keys: raw key returned once, DB stores `key_hash` + `key_prefix` only. `POST /auth/api-keys` gated by `AUTH_BOOTSTRAP_ADMIN_TOKEN`. A key with `allow_all_offices=false` must carry a non-empty `allowed_office_ids` (rejected at creation).
